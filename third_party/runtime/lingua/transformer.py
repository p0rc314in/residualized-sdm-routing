...
try:
    ...
    class _ProbeUnavailable:
        @staticmethod
        def log_stats(value, name):
            del name
            return value

    probe = _ProbeUnavailable()
...
class BaseTransformerArgs:
    ...
    # Optional constructor seam for source-compatible SDM extensions. None preserves
    # the released SparseDeltaMemory constructor exactly. This is deliberately a
    # factory rather than a model-forward or layer-forward override.
    sdm_factory: Any = None
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if attn_impl == "flex_attention":
            ...
            if fmha is None:
                raise RuntimeError("fmha attention requires the optional xformers package")
...
class TransformerBlock(nn.Module):
    def __init__(self, args: BaseTransformerArgs, layer_id: int = 0, layer_type: str = "attn"):
        ...
        if layer_type == "sdm":
            ...
            sdm_factory = args.sdm_factory or SparseDeltaMemory
            self.attention = sdm_factory(args.sdm_args, layer_id)
            self.head_dim = None
            self.n_heads = None
