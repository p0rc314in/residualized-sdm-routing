...
from .initial_memory import initial_memory_parameters
...
class SDMStatsMixin:
    ...
    def collect_grad_stats(self) -> None:
        ...
        # Initial-memory gradient (one full table or two product-key factors).
        memory_gradients = [
            parameter.grad
            for parameter in initial_memory_parameters(self)
            if parameter.grad is not None
        ]
        if memory_gradients:
            squared = sum(gradient.float().square().sum() for gradient in memory_gradients)
            self._grad_stats["memory_grad_norm"].append(squared.sqrt().item())

        # Q/K projection gradients
        qk_grad_norms = []
        if self.shared_router is None:
            routing_projections = [self.Wq_read, self.Wk_write]
        else:
            routing_projections = [
                self.shared_router.base,
                self.shared_router.read_residual,
                self.shared_router.write_residual,
            ]
        for proj in routing_projections:
            if proj is not None:
